...
> **Code Inspection Baseline:** `main` @ `daf4f98`；Phase8.1、Phase8.2、Phase8.3 Accepted，保留各自验收限制
...
Phase9 使用 9.0 前置可行性门和 9.1–9.5 五个开发子阶段。开发可在一次连续执行中完成：每个单元先写失败测试、实现、修复并回归，通过后自动继续；独立 review 集中在 9.5，不逐阶段等待人工批准或 Accepted。
...
| 9.3 游戏闭环 | [Stardew 行动与交互](./GameAgent%20MVP0%20Phase9.3%20Stardew%20行动与交互技术开发方案.md)：点位、校验、移动、等待、接近、来源、租约、UI 结束和原生恢复 | 当前运行中的真实预约行为成立；游戏侧自动化通过 |
| 9.4 恢复与认知 | [检查点与结果记忆](./GameAgent%20MVP0%20Phase9.4%20检查点与结果记忆技术开发方案.md)：保存桥接、精确恢复、结果 History、Task Context | 跨日保存/加载和故障窗口通过；后续请求含实际结果 |
| 9.5 验收与交付 | [联调验收与交付](./GameAgent%20MVP0%20Phase9.5%20联调验收与交付技术方案.md)：全量回归、真实模型/游戏、集中 review、修复和复验 | 成功赴约/爽约各连续三次，无阻断 review 问题，最终产物与证据一致 |
...
若缺少游戏环境，可先完成不依赖实机结论的 9.1/9.2 和纯逻辑测试；9.0 保持未通过，涉及真实 API 的结论不能以 fake 替代。发现跨地图、恢复或保存边界不可行时报告具体阻断，不擅自取消这些验收条件。

### 13.3 AI 连续执行指令

读取仓库 AGENTS.md、本总方案和五份 9.1–9.5 子方案，以现有工作区为基线，连续完成 Phase9。先执行 9.0 可行性检查，再按 9.1 → 9.2 → 9.3 → 9.4 → 9.5 实现。每个任务单元完成对应测试并修复失败，通过后自动继续，不逐阶段等待人工 review。独立代码/系统 review 在 9.5 集中执行，发现问题后修复并复验实际交付产物。
...
只在缺少权限、游戏环境、凭据或出现需要改变产品规则的真实阻断时请求协助；可以继续不依赖阻断的工作，但不得声称相关门槛已通过。保留用户现有修改，未经明确要求不提交或推送 git。开发过程与真实证据写入 `docs/phase9/GameAgent MVP0 Phase9 开发与验收记录.md`；总方案和子方案保持最终合同，不混入调试过程。

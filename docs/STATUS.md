...
| Runtime instance | One process loads one selected Game Profile. Profile configuration, definitions, memory/task policy, HTTP status, and gateway admission are published as one Ready instance. Choosing another game while Ready saves the next selection and requires a restart without changing current behavior. |
...
| Official Adapter repository | Stardew Valley and RimWorld live in the independent [world-is-agent-adapter](https://github.com/zlci7/world-is-agent-adapter) Git repository. Each game owns version and Protocol pins plus self-contained build, test, package, and install scripts. Stardew's 494 tests and static checks pass; RimWorld's 42 tests plus standalone build, package, temporary installation, and negative probes pass. Source is available on GitHub; joint real-game acceptance and packaged releases are pending. |

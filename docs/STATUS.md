...
| Official Adapter repository | Stardew Valley and RimWorld live in the independent local `world-is-agent-adapters` Git repository. Each game owns version and Protocol pins plus self-contained build, test, package, and install scripts. Stardew's 494 tests and static checks pass; RimWorld's 42 tests plus standalone build, package, temporary installation, and negative probes pass. GitHub upload and joint real-game acceptance are pending. |

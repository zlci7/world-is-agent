...
| RimWorld adapter | A real RimWorld 1.6 mod that talks to the Runtime in-process over `Grpc.Core`, because Unity's Mono has no HTTP/2 implementation and `grpc-dotnet` cannot run in that process. Windows x64 only. The mod is installed by `scripts/install-rimworld-adapter.ps1` and enabled in RimWorld's mod list. |

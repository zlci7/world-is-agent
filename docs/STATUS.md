...
| Stardew deferred letter | `schedule_mail` plans one letter for the next in-game morning instead of sending it now. It only produces a task proposal; the Runtime commits the durable task and wakes the NPC the next morning, when the letter is written from what the NPC knows then rather than pre-written. The Adapter owns the delivery window and the gate (one planned letter per NPC per in-game day), so the model never supplies a game tick. Ships together with `send_mail`. **Implementation and automated tests only: the real-machine closed loop has not been run yet.** |

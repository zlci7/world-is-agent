...
RimWorld adapter validation: the second real adapter was run end to end on a real machine against a live provider. A colonist gizmo produced `player_interacted_with_npc`, the Runtime asked for an observation and got a bounded `state.rimworld` projection, the model chose `present_dialogue`, the window showed that colonist's line with three replies, and the player's reply started the next turn through `player_said_to_npc`. Three colonists across two game sessions produced six turns, every one of them `steps=1`, `tools=[present_dialogue]` and `settled_by=present_dialogue`; closing a window produced none. The Runtime's Go code and the protocol were not modified for any of it. Not covered: the caravan case for entity-id stability, the negative side of the eligibility gate because the validation colony held no prisoners, guests or raiders, and a field-by-field context comparison between two colonists - the isolation evidence is per-entity turns, per-entity history sources and per-entity memory accounting rather than a context dump.

...
RimWorld adapter validation: the second real adapter was run end to end on a real machine against a live provider. A colonist gizmo produced `player_interacted_with_npc`, the Runtime asked for an observation and got a bounded `state.rimworld` projection, the model chose `present_dialogue`, the window showed that colonist's line with three replies, and the player's reply started the next turn through `player_said_to_npc`. Across sessions, entity identity held through save and reload, and two colonists left the map in a caravan and returned keeping the same `entity_id` while reporting no position off-map. The Runtime's Go code and the protocol were not modified for any of it. Not covered: the negative side of the eligibility gate - that a prisoner, guest or raider shows no WIA gizmo - was never observed, because no validation colony held one; it rests on the eligibility check in `WiaDialogueComp` and was waived rather than verified. Also not covered: a field-by-field context comparison between two colonists, where the isolation evidence is per-entity turns, per-entity history sources and per-entity memory accounting rather than a context dump.

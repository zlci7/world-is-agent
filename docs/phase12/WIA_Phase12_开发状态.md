...
当前成果包含结构化回合意图解析、明确目标与私聊隔离、NPC 协调结果与玩家叙述分层、带来源与人称代称的玩家行动投影、低影响叙事自由边界、无目标 NPC 的默认沉默与被动动作过滤、存档级叙事人称/篇幅/描写设置、行动结果到人物感知的闭环、阶段内个人上下文延续、场景人物进出与版本原子提交、按输入顺序约束的失败重试、受宿主生命周期管理的可靠另存、首页与活动世界解耦、可确认删除当前或非当前存档、原子模型配置发布、单数据根进程互斥、运行状态恢复和本地 Origin 校验。

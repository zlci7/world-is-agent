...
独立构建验证从固定 tag 导出协议源码，记录解析后的 commit，并验证 `proto/gameagent.proto` 与该版本一致。C# 类型继续由各自的 Grpc.Tools 在构建时生成；不维护可独立修改的第二份协议。
...
每款游戏独立产出安装包及清晰的本地输出路径，包名包含游戏标识与 Adapter 版本。版本和 Mod metadata 保持一致，不因迁仓自动提高版本或统一两款游戏的版本号。
...
3. 导入两个游戏目录和专属脚本，调整路径、规则、README、构建与打包入口。
4. 两个 Adapter 在没有主仓库和另一款游戏目录的环境中完成独立验证。
...
| 文件或范围 | 修改内容 |
| --- | --- |
| 主仓库 `AGENTS.md` | 固定两个仓库名称、所有权、版本依赖和统一验收方式 |
| Adapter 仓库 `AGENTS.md` | 每游戏独立构建、Runtime 内部依赖禁令、兼容标识、验证与提交规则 |
| 两个仓库 README | 各自职责、安装与开发入口、已验证范围 |
| 每游戏 README | Protocol pin、游戏依赖、构建、测试、打包和安装命令 |
...
以下为开发后的通过条件，当前尚无 10.5 执行结果。

| 验证 | 通过条件 |
| --- | --- |
| 文件清单 | 两个 Adapter 的必要源码、资源、测试与许可证完整；原 Mod 标识保持一致 |
| 固定协议版本 | 使用 `protocol-v1alpha2.0` 的已记录 commit 与 proto 内容；不依赖当前主仓库 HEAD |
...
| 协议负向探针 | 无效 `WIA_PROTOCOL_DIR` 必须因该路径失败，不能落回隐藏默认值 |
...
| 实机连接与对话 | 两个迁移后构建的 Adapter 分别连接所选游戏并完成一次对话 |
...
3. 主仓库和两个 Adapter 的独立构建与回归全部通过，固定协议依赖可复现。
4. 两款游戏的安装包经完整性检查，Runtime 便携包可脱离源码运行。

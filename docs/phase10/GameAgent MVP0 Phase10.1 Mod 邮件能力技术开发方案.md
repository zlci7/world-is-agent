...
> **Status:** 已验收（2026-09-18）：实机冒烟、回合内自主选中、跨 NPC 人设抽验均通过；负向结论由普通动机回合提供证据；完整人设采样集推迟到自主触发小阶段。逐条记录见 [Phase10 验收检查表](GameAgent%20MVP0%20Phase10%20验收检查表.md)
> **Date:** 2026-09-18
...
> **本阶段要求（已缩减，用户决策 2026-09-18）**：只做**一次抽验**——换一个未采样过的 NPC，沿用 §5.5 已生效的延迟型动机（同一条台词，控制动机变量、只变人设），按下面的四项检查项判定。合格线同「无越界承诺」一项。
>
> 下面的完整样本设计**不在本阶段要求内**，推迟到自主触发小阶段使用；保留在此是因为它是那一步的现成规格。
...
> **本阶段要求（已缩减，用户决策 2026-09-18）**：不单独跑本节的三场景集。§5.5 的普通动机回合已提供**更强**的负向证据——"关系疏远 / 临别 / 未说出口"三种动机比本节场景更接近写信，模型仍 3/3 未调用。负向结论按该证据记录，下面的场景集随完整人设采样一起推迟。

总纲 §2.2 把"能力不适用时模型不调用"列为判定类硬验收。它是这一轮的**另一半结论**：
...
5. 人设抽验一次：用一个未采样过的 NPC、沿用 §5.5 已生效的延迟型动机产出一封信，按 §5.2 的四项检查项判定通过。
6. 负向结论成立：普通动机回合中模型未调用 `send_mail`（证据见 §5.6，不再单列场景集）。
7. §5.3 全部通过；`check-architecture.ps1` 的 game-agnostic 断言未被削弱。
8. 协议与 Mod 运行时标识未改动。

> **本阶段缩减的范围（用户决策，2026-09-18）**：完整 6 封人设采样与三场景负向集**不作为本阶段要求**。负向结论由 §5.5 的普通动机回合提供证据；完整人设采样推迟到自主触发小阶段——届时信件由异步场景自然产生，样本比现在刻意构造动机更容易收，且此时采到的信件才具备对外引用价值。

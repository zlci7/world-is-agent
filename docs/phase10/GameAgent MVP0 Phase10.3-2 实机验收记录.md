...
```text
条件 6 的商队部分：Map → 商队 → Map 期间 Pawn.Map == null 且 entity_id 不变。
```

原因是**可观测的，不是推测**：验收用的殖民地来自 `-quicktest`，在该殖民地上

```text
选中殖民者后命令栏只有「WIA 对话」与「征召」两个 gizmo，没有组建商队入口；
切到世界地图单选自己的定居点，gizmo 栏也只有 dev 模式的几个世界工具与视图切换。
```

也就是说这张测试地图没有提供组建商队的入口，需要一局通过正常新开档流程建立的殖民地。
补验步骤：

```text
1  正常新开一局（含 ≥2 名殖民者）并载入，等待 [WIA] identity tick 行出现
2  选中至少一名殖民者 → 组建商队 → 让它真的出发旅行

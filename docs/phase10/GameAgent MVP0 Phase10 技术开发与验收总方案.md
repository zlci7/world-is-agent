...
| 10.2-3 | **首次运行配置**：写配置与凭据、连接测试通过才落盘、之后的 Runtime Ready —— **已验收** | 这一步做完，"外人能跑起来"才成立。依赖体检与游戏路径探测**不在本步**，见 §3.5 注 |
| 10.2-4 | 可视化：AgentTurn 时间线（复用现有 JSONL trace）、对话记录、任务与记忆查看；**运行状态可视化**（游戏是否已连接、能力是否可用） | 这是"好看"，前三步才是"能用" |

注意 10.2-2 与 10.2-4 共用同一个 HTTP 面：**不需要为"控制面"和"UI 接口"各做一套。**

**10.2-3 的范围在开工后收窄过一次，这里记录结论而不是过程。** 依赖体检（.NET / SMAPI / 游戏路径 / MFM 探测）与"游戏是否已连接"的状态呈现**移出 10.2-3，归 10.2-4**：前者要 Game Runtime 去当 Stardew 安装诊断器，后者要 gateway 开放只读世界状态，两者都不服务于"用户能不能把 Runtime 配起来"这个唯一问题。10.2-3 只做：seed 默认配置、存凭据、连接测试、保存并进入 Ready、一个配置页。相应地 §3.7 的退出条件 4 在 10.2-3 只兑现前半句。

**10.2-1 已实现**（见 §3.2.1、§3.2.2）。**10.2-2 的首个切片已实现并实机验证**：控制面、凭证交接、`/api/status`、`/api/turns`、资产内嵌与自动打开浏览器；服务端不提供 `health` 路由——它没有消费者，且总方案 §3.4 要求 `/api` 全部带凭证，一个免凭证的健康检查与该约束直接冲突。**10.2-3 已验收**：空数据根 seed、凭据 `file:` 引用、探测通过才落盘、三态状态（`needs_configuration` / `blocked` / `ready`）、首次配置页与真机端到端均通过，见 [10.2-3 首次运行配置技术开发方案](GameAgent%20MVP0%20Phase10.2-3%20首次运行配置技术开发方案.md) §10.1、§10.2。10.2-4 与发行步骤尚未开工。
...
4. 首次运行配置能产出可用配置（10.2-3）；对缺失依赖给出可操作的明确提示归 10.2-4。

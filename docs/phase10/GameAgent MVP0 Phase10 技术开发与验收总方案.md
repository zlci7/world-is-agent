...
| 10.2-3 | **首次运行配置**：写配置与凭据、连接测试通过才落盘、之后的 Runtime Ready —— **已验收** | 这一步做完，"外人能跑起来"才成立 |
| 10.2-4 | **发行形态**：Portable ZIP、版本号、仓库外零依赖运行验证 | 退出条件 3 的唯一兑现方式。这一步做完，10.2 才可以正式验收 |
| 10.2-5 | **运行状态可视化**：AgentTurn 时间线细化、对话记录、任务与记忆查看、游戏是否已连接、能力是否可用、依赖体检 | **Post-acceptance enhancement，不阻塞 10.2 验收。** 页面是给玩家用的：能跑的闭环不依赖这些查看面，只有需要排查的玩家才需要 |

注意 10.2-2 与 10.2-5 共用同一个 HTTP 面：**不需要为"控制面"和"UI 接口"各做一套。**

**10.2-3 的范围在开工后收窄过一次，这里记录结论而不是过程。** 依赖体检（.NET / SMAPI / 游戏路径 / MFM 探测）与"游戏是否已连接"的状态呈现**移出 10.2-3**：前者要 Game Runtime 去当 Stardew 安装诊断器，后者要 gateway 开放只读世界状态，两者都不服务于"用户能不能把 Runtime 配起来"这个唯一问题。10.2-3 只做：seed 默认配置、存凭据、连接测试、保存并进入 Ready、一个配置页。

**10.2-4 与 10.2-5 的分工在 10.2-3 验收后确定。** 原本把它们合为一步"可视化"，但那会把"能不能交付给别人"和"页面好不好排查"绑在同一道门里：前者是产品成立的必要条件，后者只服务于需要排查的玩家——这个页面是给玩家用的，正常玩并不需要 turn 详情、任务或记忆查看。因此发行形态独立为 10.2-4（退出条件 3 的唯一兑现方式），其余查看面归 10.2-5，**不阻塞 10.2 验收**。相应地 §3.7 的退出条件 4 只要求"首次运行配置能产出可用配置"，依赖提示不再挂在 10.2 的出口上。

**10.2-1 已实现**（见 §3.2.1、§3.2.2）。**10.2-2 的首个切片已实现并实机验证**：控制面、凭证交接、`/api/status`、`/api/turns`、资产内嵌与自动打开浏览器；服务端不提供 `health` 路由——它没有消费者，且总方案 §3.4 要求 `/api` 全部带凭证，一个免凭证的健康检查与该约束直接冲突。**10.2-3 已验收**：空数据根 seed、凭据 `file:` 引用、探测通过才落盘、三态状态（`needs_configuration` / `blocked` / `ready`）、首次配置页与真机端到端均通过，见 [10.2-3 首次运行配置技术开发方案](GameAgent%20MVP0%20Phase10.2-3%20首次运行配置技术开发方案.md) §10.1、§10.2。
...
3. 免安装包解压后可直接运行；不需要管理员权限、不需要安装步骤。**已兑现**：`scripts/release-runtime.ps1` 产出 `world-is-agent-v0.1.0-windows-amd64.zip`（7.4 MB，含 `wia-runtime.exe` / `README.txt` / `LICENSE`），已在仓库外目录解压运行验证——零参数启动、seed 35 个 definition、内嵌客户端可服务、`/api/status` 报 `version=0.1.0`。
4. 首次运行配置能产出可用配置（10.2-3，已验收）。对缺失依赖给出可操作的明确提示归 10.2-5，不阻塞本阶段。
...
- 10.1 的实机验证与后续的依赖体检共用"检查依赖是否就绪"的逻辑，避免重复实现；依赖体检已移出 10.2-3，见 §3.5 注。

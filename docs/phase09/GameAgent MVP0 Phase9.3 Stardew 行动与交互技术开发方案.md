...
- [x] 连续运行下预约 → 出发 → 等待 → met/expired 的真实行为成立，玩家无需跟随。见证记录第 9.1 节：met 与 expired 各一条完整链路。
- [ ] 沙滩、酒馆、广场的生产点位、受支持路线、行程预留和实际加载预算均有对应证据；未验证路线保持不可用。**未按原范围交付**：沙滩保留固定路线与实测预留，酒馆与广场未采集；新增五个用户实机标注点使用通配路线且未记录实测行程。范围变更与代价见记录第 9.2 节。
- [x] 四个能力、来源和租约测试通过；旧 move_to / 对话/取消回归通过。
- [x] NPC 恢复原生行为有真实移动证据，而非仅日志和 controller=null。见证记录第 3 节的三次 `native_schedule_rejoined`；新增点位当前只有终态释放记录与实机观察。
- [ ] 无重复终态、第三方控制器误清理或永久停留。终态唯一性有实机证据（两条闭环各产生一条终态事实）；第三方 controller 接管只有单元测试，未做实机注入。

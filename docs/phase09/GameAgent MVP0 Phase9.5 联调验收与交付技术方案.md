...
> **执行方式:** 9.1–9.4 的内部 CR、阶段级用户审查和验收通过后，执行最终联调、整分支/系统审查、修复和复验。本阶段新增或修正模块完成测试后本地提交并连续推进；阶段代码完成后统一执行 agent 内部 CR，修正并复验，再由用户集中 CR、review 和最终实机验收。
...
- 每个独立模块先完成失败测试、实现、相关测试及直接受影响的回归，运行 `git diff --check` 后本地提交并继续。各子阶段代码完成后执行整体自动化回归和阶段级内部 review，修正使用独立 `fix:` 本地提交并复验，再交付用户统一审查与验收。
...
- 本阶段在全部模块提交后执行最终整分支/系统审查，处理发现的问题并复验，再交付用户集中 CR、review 和真实游戏验收；模块内的聚焦测试检查本模块行为，最终审查检查跨模块合同与交付产物，不替代 9.3/9.4 的阶段验收。

...
Require-Content 'src/Integrations/MailFramework/MailFrameworkIntegration.cs' 'MailDeliveryGuard\.ShouldDeliver' 'The mail condition must go through the delivery guard; MFM has no Repeatable check on the API path.'
Require-Content 'src/Integrations/MailFramework/MailFrameworkIntegration.cs' 'ulong registeredSaveId = Game1\.uniqueIDForThisGame;' 'The registered save identity must be a per-registration local, so each letter keeps the save that created it.'
Require-Content 'src/Integrations/MailFramework/MailDeliveryGuard.cs' 'registeredSaveId == currentSaveId && !alreadyRead' 'A registered letter must not be delivered into a different save; MFM''s repository outlives the save.'
Reject-Content 'src/Integrations/MailFramework/MailFrameworkIntegration.cs' 'private .*registeredSaveId' 'A save identity field would make every earlier letter follow the newest save.'

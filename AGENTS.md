...
当前已授权的产品方向是面向普通玩家的一体化 AI 叙事角色扮演应用。开发入口为 [Phase12 开发执行指南](docs/phase12/WIA_Phase12_开发执行指南.md)，并以其关联的产品说明、技术方案及开发状态接续工作。以下历史 MVP0、Protocol 与 Adapter 约束用于理解历史代码；与 Phase12 已确认范围冲突时，按当前叙事产品要求实施，不将其作为重构的兼容前提。

- 现有代码可围绕叙事产品重构、替换和移除。目标为一个正式叙事后端入口、一套玩家前端；不设计 Text Adapter，不要求保留外部游戏接入能力。
- 产品支持场景主 Agent 与重要 NPC Agent 协作、个人感知与记忆、条件化世界剧情、开放型与流程型剧本。此处是产品内的角色协作要求，不是自动授权开发工具启动子 agent。
- 保留 game_id / world_id / entity_id 身份与 definition_id 模板概念。每个世界独立数据库，正常游玩持续持久化，另存才创建新世界，读取后继续更新所选世界。
- 接受少量不影响持续游玩的叙事幻觉；数据所有权、版本、事务及存档正确性按程序合同验证。不为每段正文设置固定语义裁判流程。
- 用户按大阶段体验验收；内部小步骤由开发会话自行实施、验证和修复，不逐项请求确认。每个独立小工作单元完成并通过相关检查后，已授权执行一次本地 git commit，无需再次询问；不执行 git push。执行范围以用户本次授权的阶段为准，实际进度如实记录。
- 提交按功能或可独立说明的改动组织，只包含本工作单元相关代码、测试和文档，不混入用户无关改动、秘密或运行数据。所有权、普通用户文件访问和秘密保护等通用要求继续有效；代码重构不授权删除用户数据、操作其他仓库或发布生产服务。
- 设计文档不代表已实现。公开事实源随实际交付更新，原有实现事实与 Phase12 目标明确区分。
...
- 主仓库名称为 `world-is-agent`；官方 Adapter 的本地目录为 `world-is-agent-adapters`，GitHub 远端为 `https://github.com/zlci7/world-is-agent-adapters.git`。
...
- Phase12 已获按完成的小工作单元持续执行本地 `git commit` 的授权；其他任务在用户明确授权后提交。均不执行 `git push`。
